...
exports.verifyOTP = async (req, res, next) => {
  try {
    ...
    if (!otpToken || isOtpEmpty || otp.length < 6)
      throw new CustomError("Invalid OTP", 400);

    const emailExist = await User.findOne({ email }).exec();
    if (!emailExist || !emailExist.active)
      throw new CustomError(
        "Your account has been blocked. Access has been prohibited for security reasons.",
        403,
    ...
    if (!decoded || decoded.otp !== otp) {
      ...
      if (
        ...
        throw new CustomError(
          "You've tried too many times with an incorrect OTP, this account has been temporarily blocked for security reasons. Please reach out to our Technical Support team for further assistance.",
          429,
          { otpVerified: isOtpVerified }
        );
      }
