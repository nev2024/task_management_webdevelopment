...
const options = {
  host_whitelist: ["gmail.com", "yahoo.com", "outlook.com", "controlapp.com"],
};
...
const validateAuthInput = (fieldName, value) => {
  ...
  if (fieldName.toLowerCase() === "email" && !validator.isEmail(value, options))
  ...
  if (
    ...
    throw new CustomError("Password not strong enough", 400);
};

const validateObjectId = (id, idType) => {
  if (validator.isEmpty(id, { ignore_whitespace: true }))
    throw new CustomError(`${idType} id required`, 400);

  if (!mongoose.Types.ObjectId.isValid(id))
    throw new CustomError(`No such ${idType.toLowerCase()} id found`, 404);
};
