...
const Note = () => {
  ...
  useEffect(() => {
    ...
    const getNoteList = async () => {
      try {
        const endpoint =
          targetUser?.userId && admin ? "/api/notes/admin-all" : "/api/notes";
        const method = targetUser?.userId && admin ? "post" : "get";
        const data =
          targetUser?.userId && admin ? { id: targetUser.userId } : undefined;
  ...
  const filteredNote = useMemo(
    () =>
      notes?.filter((note) => {
        ...
        return (
          (titles === "" ||
            note.title.toLowerCase().includes(titles.toLowerCase())) &&
  ...
  const handleBack = () => {
    setTitle("Sistemi i Menaxhimit te Detyrave");
    navigate("/");
  };
  ...
  return (
    <>
      ...
      <div className="row">
        ...
        {!filteredNote?.length && (titles || tag.length !== 0) && (
          <div>Nuk u gjetën rezultate që përputhen...</div>
        )}
      </div>

      {notFound && !titles && !tag.length && !notes?.length && (
        <div>Nuk u gjet asnjë regjistrim...</div>
      )}
    </>
  );
};
